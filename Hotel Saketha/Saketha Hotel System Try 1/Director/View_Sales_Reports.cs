...
{
    public partial class View_Sales_Reports : Form
    {
        ...
        {
            conn.Open();
            string selectQuary = "Select * from sales_report where date = '" + datepick.Value.ToShortDateString() + "' ";
            ...
            if (mdr.Read())
            {
                ...
                {
                    MySqlDataAdapter sqlAdapter = new MySqlDataAdapter("Select * from sales_report where date = '" + datepick.Value.ToShortDateString() + "' ", conn);
                    DataTable dt = new DataTable();
                    sqlAdapter.Fill(dt);
                    dataGridView1.DataSource = dt;
